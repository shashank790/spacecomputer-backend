...
exports.validateCTRNGRequest = validateCTRNGRequest;
...
exports.sanitizeCTRNGRequest = sanitizeCTRNGRequest;
 ...
 */
function validateCTRNGRequest(request) {
    const errors = [];
    ...
    if (request.src === 'ipfs') {
        ...
        if (ipfsRequest.beaconPath) {
            if (typeof ipfsRequest.beaconPath !== 'string' ||
                (!ipfsRequest.beaconPath.startsWith('/ipns/') &&
                    !ipfsRequest.beaconPath.startsWith('/ipfs/'))) {
                errors.push('beaconPath must be a valid IPFS/IPNS path starting with /ipns/ or /ipfs/');
            }
        }
        if (ipfsRequest.index !== undefined) {
            if (typeof ipfsRequest.index !== 'number' ||
        ...
        const hasIpfsParams = 'beaconPath' in request || 'index' in request || 'block' in request;
        if (hasIpfsParams) {
            errors.push("IPFS-specific parameters (beaconPath, index, block) can only be used with src: 'ipfs'");
        }
    }
    return {
        valid: errors.length === 0,
...
function sanitizeConfig(config) {
    ...
    if (!validation.valid) {
        throw (0, errors_1.createValidationError)('Invalid configuration', validation.errors);
    }
    return {
        ...
        ipfs: {
            ...
            timeout: 30000,
            enableFallback: true,
            defaultBeaconPath: '/ipns/k2k4r8pigrw8i34z63om8f015tt5igdq0c46xupq8spp1bogt35k5vhe',
            ...config.ipfs,
        },
    };
}
...
function sanitizeRequestOptions(options = {}) {
    ...
    if (!validation.valid) {
        throw (0, errors_1.createValidationError)('Invalid request options', validation.errors);
    }
    return {
 ...
 */
function sanitizeCTRNGRequest(request) {
    const validation = validateCTRNGRequest(request);
    if (!validation.valid) {
        throw (0, errors_1.createValidationError)(validation.errors.join(', '), validation.errors);
    }
